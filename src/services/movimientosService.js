...
const getSaldoCaja = async (empleadoId) => {
  try {
    ...
    // SOLO sumar ingresos y restar egresos (EXCLUIR apertura y cierre)
    ...
    // Solo monto apertura + ingresos - egresos (apertura y cierre NO se incluyen)
...
const registrarMovimiento = async (tipo, monto, descripcion, empleadoId) => {
  ...
  try {
    ...
    if (tipo.toLowerCase() === 'apertura') {
      ...
      let saldoActual = monto; // Por defecto usar el monto proporcionado
      ...
      if (ultimaCajaResult.rows.length > 0 && ultimaCajaResult.rows[0].monto_cierre) {
        // Si hay una caja cerrada anterior, usar su monto de cierre como saldo actual
        saldoActual = parseFloat(ultimaCajaResult.rows[0].monto_cierre);
      }

      // Crear nueva caja con el saldo actual como monto de apertura
      ...
      // Registrar movimiento de apertura (NO afecta el saldo - solo es registro)
    ...
    } else if (tipo.toLowerCase() === 'cierre') {
      // Obtener caja activa
      const cajaResult = await client.query(
        `SELECT idcaja, monto_apertura 
         FROM caja 
      ...
      // Calcular saldo actual (monto apertura + ingresos - egresos)
      ...
      // Actualizar caja a estado Cerrado con el saldo actual como monto de cierre
      await client.query(
        `UPDATE caja 
      ...
      // Registrar movimiento de cierre (NO afecta el saldo - solo es registro)
      const movimientoResult = await client.query(
        `INSERT INTO movimiento_caja (caja_id, tipo, descripcion, monto, empleado_id) 
      ...
      // Registrar movimiento de ingreso (SÍ afecta el saldo)
      const movimientoResult = await client.query(
        `INSERT INTO movimiento_caja (caja_id, tipo, descripcion, monto, empleado_id) 
      ...
      // Registrar movimiento de egreso (SÍ afecta el saldo)
      const movimientoResult = await client.query(
        `INSERT INTO movimiento_caja (caja_id, tipo, descripcion, monto, empleado_id) 
